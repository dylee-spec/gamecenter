-- V3: spectator list in room state and player -> spectator switching ('spectate' action).
-- Additive and backward compatible with client '2'. Apply after upgrade-v2.sql.
begin;
...
begin
 ...
 if p_action='create' then
   ...
   if p_action='start' then
     ...
     if me.id is distinct from r.black and me.id is distinct from r.white then raise exception '이미 관전 중입니다.'; end if;
     if r.status='playing' then raise exception '대국 중에는 관전으로 바꿀 수 없어요. 대국이 끝난 뒤 다시 시도해 주세요.'; end if;
     update gomoku_private.rooms set black=case when black=me.id then null else black end,white=case when white=me.id then null else white end,revision=revision+1,updated=now() where id=rid returning * into r;
   elsif p_action not in ('join','state','spectate') then raise exception '지원하지 않는 요청입니다.';
   end if;
 end if;
 ...
 return jsonb_build_object('id',r.id,'title',r.title,'locked',r.password_hash is not null,'status',r.status,'board',r.board,'turn',r.turn,'moves',r.moves,'last_cell',r.last_cell,'winner',r.winner,'winner_name',r.winner_name,'reason',r.reason,
   ...
   'spectator_list',(select coalesce(jsonb_agg(jsonb_build_object('name',p.nickname,'me',m.person=me.id) order by m.joined,m.person),'[]'::jsonb) from gomoku_private.members m join gomoku_private.people p on p.id=m.person where m.room=r.id and m.person is distinct from r.black and m.person is distinct from r.white),
